...
namespace detail {
    template <typename T>
    concept signverify_func_input =
            basic_char<typename T::value_type> && (char_view_type<T> || const_span_type<T>);
    ...
    template <typename Func, typename F = std::remove_reference_t<Func>>
    using function_signature_t = typename strip_function_object<F>::type;

    template <typename arg>
    struct parameters {};

    template <typename F, signverify_func_input arg>
    struct parameters<F(arg)> {
        using type = arg;
    };

    template <typename F, signverify_func_input arg>
    struct parameters<F(arg, arg)> {
        using type = arg;
    };
    ...
    template <typename F, signverify_func_input... arg>
    struct result<F(arg...)> {
        using type = std::invoke_result_t<F, function_signature_t<F>>;
    };

    template <typename Func, typename F = std::remove_reference_t<Func>>
    using function_return_t = typename result<F>::type;

    template <typename Func, typename F = std::remove_reference_t<Func>>
    constexpr auto parameter_type(Func&) -> typename strip_function_object<F>::type;

    template <typename Func, typename F = std::remove_reference_t<Func>>
    concept void_return_func = lambda_function<F> && std::is_void_v<function_return_t<F>>;
    ...
    struct bt_serialize<T> {
        void operator()(std::ostream& os, const T& val) {
            bt_serialize<std::string_view>{}(os, detail::span_to_sv(val));
        }
    };
...
class bt_list_consumer {
  ...
  public:
    ...
    bt_list_consumer(T input) :
            bt_list_consumer{
                    reinterpret_cast<const char*>(input.data()), input.size(), load_tag{}} {}
    ...
    void consume_signature(VerifyFunc verify) {
        using InputT = decltype(parameter_type(verify));
...
class bt_dict_consumer : private bt_list_consumer {
    ...
    virtual void _init_internals() override {
        if (data.empty())
  ...
  public:
    ...
    void consume_signature(VerifyFunc verify) {
        ...
        if constexpr (detail::char_view_type<CharT>)
