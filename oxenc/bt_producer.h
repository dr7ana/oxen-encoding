...
namespace detail {
    ...
    // std::string_view to_sv(const std::basic_string_view<CharT>& x) {
    //     return {reinterpret_cast<const char*>(x.data()), x.size()};
    // }

    template <typename SignFunc, typename InputT>
    concept sign_func = signverify_func_input<InputT> && std::is_invocable_v<SignFunc, InputT>;

    template <typename Class, typename SignFunc>
    auto append_signature_helper(Class& self, SignFunc sign) {
        using InputT = decltype(parameter_type(sign));
        using CharT = typename InputT::value_type;

        if constexpr (char_view_type<InputT>)
            return sign(self.template view_for_signing<CharT>());
        else if constexpr (const_span_type<InputT>)
            return sign(self.template span_for_signing<CharT>());
        else
            throw std::invalid_argument{
                    "Signing function requires view or const-span type argument!"};
    }
}  // namespace detail
